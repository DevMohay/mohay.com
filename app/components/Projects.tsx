...
const PROJECTS_DATA: Project[] = [
  {
    name: "mehaxan",
    description:
      "Personal portfolio website built with Angular 21 and deployed on Cloudflare Workers. You're looking at it right now!",
    technologies: [
      "Angular",
      "TypeScript",
      "Tailwind CSS",
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1400px] mx-auto px-[var(--shell-pad-x)] flex flex-col gap-[clamp(2.5rem,6vw,4rem)]">
        ...
        >
          <div className="w-full">
            ...
            <h2 className="text-[clamp(2.5rem,5vw,4.5rem)] font-[350] tracking-[-0.03em] leading-[1.05] font-display mt-2">
              <span className="italic text-[var(--accent)]">Apps</span> I shipped to the Store <br />
              <span >Sites</span>
              <span className="ml-6">I pushed to <span className="italic text-[var(--accent)]">production</span> </span> built — because the idea
        ...
        >
          <div
            className="flex-shrink-0 w-[var(--shell-pad-x)]"
            aria-hidden="true"
          />
          ...
          {featuredProjects.map((project) => (
            <article
              key={project.name}
              className="panel flex-shrink-0 w-[min(86vw,64rem)] scroll-snap-start bg-[var(--bg-card)] border border-[var(--line)] overflow-hidden grid grid-cols-1 lg:grid-cols-[1.05fr_1fr] min-h-[28rem] transition-colors hover:border-[var(--line-strong)] shadow-[var(--shadow-soft)]"
            >
              <div className="relative bg-[var(--bg-surface)] overflow-hidden">
                ...
                <span className="absolute top-4 left-4 px-3 py-1.5 font-mono text-[0.66rem] tracking-[0.16em] uppercase text-[var(--accent-ink)] bg-[var(--accent)] rounded-full">
                  Featured
                </span>
              </div>
              ...
              <div className="p-[clamp(1.75rem,3vw,2.75rem)] flex flex-col justify-between gap-4">
                <div className="flex flex-col gap-4">
                  <p className="font-mono text-[0.72rem] tracking-[0.16em] uppercase text-[var(--text-muted)] m-0">
                    ...
                    <span className="mx-2 opacity-50">/</span> Case study
                  </p>
                  <h3 className="text-[clamp(2rem,3.6vw,3rem)] font-[350] tracking-[-0.03em] leading-[1.05] m-0 font-display">
                    {project.name}
                  </h3>
                  <p className="text-[var(--text-secondary)] text-base leading-relaxed max-w-[48ch] m-0">
                    {project.description}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="px-3 py-1 bg-[rgba(244,241,234,0.06)] border border-[var(--line)] rounded-full text-xs font-medium text-[var(--text-secondary)]"
                      >
                        {tech}
                      </span>
                    ))}
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  {project.url && (
                    <a
                      ...
                      onClick={onCardClick}
                      className="inline-flex items-center gap-2 px-4 py-2.5 bg-[var(--bg-inverse)] text-[var(--text-inverse)] rounded-full text-[0.85rem] font-medium transition-all hover:bg-[var(--accent)] hover:text-[var(--accent-ink)] hover:-translate-y-0.5"
                    >
                      Visit live
                      <svg
                        width="14"
                  ...
                  {project.github && (
                    <a
                      ...
                      rel="noopener noreferrer"
                      onClick={onCardClick}
                      className="inline-flex items-center gap-2 px-4 py-2.5 bg-transparent text-[var(--text-primary)] border border-[var(--line-strong)] rounded-full text-[0.85rem] font-medium transition-all hover:bg-[var(--text-primary)] hover:text-[var(--bg-card)] hover:border-[var(--text-primary)]"
                    >
                      Source
                      <svg
                        width="14"
